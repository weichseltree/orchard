...
describe("VenueCurtains", () => {
  ...
  it("hangs one curtain per door into a room that asks more than the room before it", () => {
    expect(curtains.doors).toEqual(["curtain-foyer-club", "curtain-club-stage"]);
  });

  it("shows a curtain only over a barred door, and animates only what shows", () => {
    curtains.update(1, new Set(["club", "stage"]));
    expect(curtains.group.children.map((c) => c.visible)).toEqual([true, true]);
    curtains.update(2, new Set(["stage"]));
    expect(curtains.group.children.map((c) => c.visible)).toEqual([false, true]);
  ...
  it("stands in the doorway at the door's own floor", () => {
    ...
    expect([club.position.x, club.position.y, club.position.z]).toEqual([10, -3, -22.5]);
    const stage = curtains.group.children[1]!;
